...
class PolySettings(BaseSettings):
    ...
    target_notional_per_opp: float = Field(default=25.0)
    max_notional_per_opp: float = Field(default=250.0)
    min_depth_edge_per_share: float = Field(default=0.0)
    market_cooldown_seconds: float = Field(default=60.0)
    local_book_depletion_ttl_seconds: float = Field(default=900.0)
    max_market_notional_per_day: float = Field(default=50.0)
    max_daily_notional: float = Field(default=250.0)
    ...
    book_source: str = Field(default="http")
    ...
    ws_market_url: str = Field(default="wss://ws-subscriptions-clob.polymarket.com/ws/market")
    ws_heartbeat_seconds: int = Field(default=10)
    ws_reconnect_min_seconds: float = Field(default=1.0)
    ws_reconnect_max_seconds: float = Field(default=30.0)
    ws_ready_timeout_seconds: float = Field(default=15.0)
    ws_min_ready_ratio: float = Field(default=0.95)
    ws_subscribe_batch_size: int = Field(default=500)
    ...
    ws_reconcile_max_tokens_per_cycle: int = Field(default=500)
    ...
    storage_async_flush_seconds: float = Field(default=1.0)
    book_top_sample_seconds: float = Field(default=0.0)
