...
FUTU_MARKET_FLOW_RATE_LIMIT_RETRY_ATTEMPTS="${FUTU_MARKET_FLOW_RATE_LIMIT_RETRY_ATTEMPTS:-2}"
FUTU_MARKET_FLOW_RATE_LIMIT_RETRY_SECONDS="${FUTU_MARKET_FLOW_RATE_LIMIT_RETRY_SECONDS:-31}"
FUTU_MARKET_FLOW_TRANSIENT_RETRY_ATTEMPTS="${FUTU_MARKET_FLOW_TRANSIENT_RETRY_ATTEMPTS:-2}"
FUTU_MARKET_FLOW_TRANSIENT_RETRY_SECONDS="${FUTU_MARKET_FLOW_TRANSIENT_RETRY_SECONDS:-5}"
...
FUTU_MARKET_FLOW_EXCLUDE_SECURITY_CLASSES="${FUTU_MARKET_FLOW_EXCLUDE_SECURITY_CLASSES:-preferred,note_debt,unit,warrant_right,delisted_label,future_listing}"
...
FUTU_MARKET_FLOW_OVERWRITE="${FUTU_MARKET_FLOW_OVERWRITE:-false}"
...
US_OTC_PROXY_FLOW_CONCURRENCY="${US_OTC_PROXY_FLOW_CONCURRENCY:-1}"
...
SCAN_ARGS=(
    ...
    --rate-limit-retry-attempts "$FUTU_MARKET_FLOW_RATE_LIMIT_RETRY_ATTEMPTS"
    --rate-limit-retry-seconds "$FUTU_MARKET_FLOW_RATE_LIMIT_RETRY_SECONDS"
    --transient-retry-attempts "$FUTU_MARKET_FLOW_TRANSIENT_RETRY_ATTEMPTS"
    --transient-retry-seconds "$FUTU_MARKET_FLOW_TRANSIENT_RETRY_SECONDS"
    ...
    --exclude-security-classes "$FUTU_MARKET_FLOW_EXCLUDE_SECURITY_CLASSES"
)

if [ "$FUTU_MARKET_FLOW_INCLUDE_DISTRIBUTION" = "true" ]; then
    SCAN_ARGS+=(--include-distribution)
fi
if [ "$FUTU_MARKET_FLOW_OVERWRITE" = "true" ]; then
    SCAN_ARGS+=(--overwrite)
fi
...
if [ "$ENABLE_US_OTC_PROXY_FLOW" = "true" ] && market_list_contains "$FUTU_MARKET_FLOW_MARKETS" "US"; then
    ...
    if [ "$US_OTC_PROXY_FLOW_PROVIDER" = "polygon" ] && [ -z "${POLYGON_API_KEY:-}" ] && [ -z "${POLYGON_API_KEY_FILE:-}" ]; then
        ...
        US_OTC_PROXY_ARGS=(
            ...
            --concurrency "$US_OTC_PROXY_FLOW_CONCURRENCY"
        )
