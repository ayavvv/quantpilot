...
PROMOTION_HORIZON = int(os.environ.get("US_MICROSTRUCTURE_PROMOTION_HORIZON", "5"))
HIGH_SCORE = float(os.environ.get("US_MICROSTRUCTURE_HIGH_SCORE", "85"))
...
def _send_email(subject: str, body: str) -> str:
    """Best-effort SMTP send reusing reporter/.env. Returns a status string."""
...
def _accrual_rate(events: pd.DataFrame, side: str) -> tuple[float, int, int]:
    ...
    if events.empty or "side" not in events.columns:
        return 0.0, 0, 0
    span = max(1, int(events["signal_date"].nunique()))
...
def _verdict(crit: dict, m: dict, matured_obs: int) -> tuple[str, bool]:
    """Return (human verdict, alert?) for a side from its matured 5d metrics."""
    if not m:
        return "⏳ 预热中：还没有成熟的 5 日样本", False
    obs = int(m.get("observation_count") or 0)
    days = int(m.get("signal_day_count") or 0)
    alpha = float(m.get("avg_alpha") or 0.0)
    hit = float(m.get("hit_rate") or 0.0)
    wilson = float(m.get("wilson_lower") or 0.0)
    share = float(m.get("max_symbol_sample_share") or 1.0)

    need_obs = int(crit.get("min_observations_per_side") or 40)
    need_days = int(crit.get("min_signal_days_per_side") or 10)
    min_alpha = float(crit.get("min_alpha") or 0.0075)
    min_hit = float(crit.get("min_hit_rate") or 0.58)
    min_wilson = float(crit.get("min_wilson_lower") or 0.50)
    max_share = float(crit.get("max_symbol_sample_share") or 0.20)
    ...
    if obs >= need_obs and ("样本数" not in failed):
...
def _eta_text(rate: float, matured_obs: int, need_obs: int, need_days: int) -> str:
    if rate <= 0:
        return "ETA: 当前无官方事件累积，无法估算"
    gate_days = max(math.ceil((need_obs - matured_obs) / rate), need_days)
    total = gate_days + PROMOTION_HORIZON + 1  # +maturation lag for the last needed signal
    weeks = round(total / 5, 1)
    return f"ETA: 按 ~{rate:.2f} 事件/交易日 → 约 {total} 交易日（~{weeks} 周）攒满样本"
...
def main() -> int:
    ...
    for side, cn in (("distribution", "派发"), ("accumulation", "吸筹")):
        ...
        matured_obs = int(m.get("observation_count") or 0) if m else 0
        need_obs = int(crit.get("min_observations_per_side") or 40)
        need_days = int(crit.get("min_signal_days_per_side") or 10)
        verdict, side_alert = _verdict(crit, m, matured_obs)
        alert = alert or side_alert
        sv = "✅validated" if validated_sides.get(side) else "warmup"
        if m:
            detail = (
                f"  {cn}侧[{sv}]: 成熟样本 {matured_obs}/{need_obs} · 信号日 "
                f"{int(m.get('signal_day_count') or 0)}/{need_days} · 命中 {_fmt_pct(m.get('hit_rate'))} · "
                f"alpha {_fmt_pct(m.get('avg_alpha'))} · Wilson下界 {float(m.get('wilson_lower') or 0):.2f} · "
        ...
        lines.append(f"     {_eta_text(rate, matured_obs, need_obs, need_days)}")
        ...
        history_row[f"{side}_signal_days"] = m.get("signal_day_count") if m else None
    ...
    # Append trend history (best-effort)
    try:
        hist_path = Path(os.environ.get("US_MICROSTRUCTURE_PROMOTION_HISTORY", str(BASE_DIR / "validation" / "promotion_tracker_history.csv")))
        ...
        if hist_path.exists():
            try:
                ...
                pass
        row_df.to_csv(hist_path, index=False)
        (VALIDATION_DIR / "promotion_tracker_latest.txt").write_text(message + "\n", encoding="utf-8")
    ...
    heartbeat_weekday = int(os.environ.get("US_MICROSTRUCTURE_PROMOTION_HEARTBEAT_WEEKDAY", "1"))
    ...
    # Exit code 10 signals "alert-worthy" so the wrapper can choose to push only on alerts.
