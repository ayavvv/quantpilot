...
def validation_end_date(report_date: str) -> str:
    """Default validation cutoff before the report date.

    The daily wrapper runs validation before writing the current report, so it
    should not consume a same-date signal file that may have been generated by
    an intraday/manual report.
    """

    return (datetime.strptime(report_date, "%Y-%m-%d").date() - timedelta(days=1)).isoformat()
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    validation = subparsers.add_parser("validation-end-date")
    validation.add_argument("--report-date", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "default-report-date":
        ...
        return 0
    if args.command == "validation-end-date":
        print(validation_end_date(args.report_date))
        return 0
