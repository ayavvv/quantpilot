...
ENABLE_STEALTH_MONEY="${ENABLE_STEALTH_MONEY:-true}"
...
if [ -n "$NAS_HOST" ] && [ -n "$NAS_USER" ] && [ "$SKIP_NAS_SYNC" != "true" ]; then
...
if [ "$ENABLE_MAJOR_MONEY_DIGEST" = "true" ]; then
    ...
    if PYTHONPATH="$PYTHONPATH" \
        "$PYTHON_BIN" -m scripts.build_major_money_digest \
            ...
            --archive-dir "$MAJOR_MONEY_DIGEST_ARCHIVE_DIR"; then
