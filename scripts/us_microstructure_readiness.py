...
def check_validation_gate(base_dir: str | Path) -> dict[str, Any]:
    ...
    return {
        ...
        "official_min_event_score": float(payload.get("official_min_event_score") or 0.0) if payload else 0.0,
        "reportable_min_event_score": float(payload.get("reportable_min_event_score") or 0.0) if payload else 0.0,
        "score_thresholds": payload.get("score_thresholds", []) if payload else [],
        ...
        "exploration_min_event_score": float(payload.get("exploration_min_event_score") or 0.0) if payload else 0.0,
        "exploration_event_count": int(payload.get("exploration_event_count") or 0) if payload else 0,
        "exploration_forward_return_count": int(payload.get("exploration_forward_return_count") or 0) if payload else 0,
